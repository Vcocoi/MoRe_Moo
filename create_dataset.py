...
class office_Dataset(Dataset):
    def __init__(self, dataset, root_path, task, mode, balanced=False):
        ...
        if balanced:
            list_path = os.path.join(BASE_DIR, 'data_txt', f'{dataset}-balanced', f'{task}_{mode}.txt')
        # or the original dataset
        else:
            list_path = os.path.join(BASE_DIR, 'data_txt', dataset, f'{task}_{mode}.txt')
        f = open(list_path, 'r') # reads the image list from text file
